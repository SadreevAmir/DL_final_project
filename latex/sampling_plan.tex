...
В \code{score_training/sde.py} уже есть deterministic VP/DDIM-like sampler. На каждом шаге:
\[
    \hat x_0 =
...
\]
После этого сделать обычный VP/DDIM step, но уже из \(\hat x_0^{\mathrm{corr}}\).
...
\]
Дальше VP/DDIM step использует \(\hat x_0^{\mathrm{phys}}\).
